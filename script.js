document.addEventListener("DOMContentLoaded", () => {
  let balance = 10000;
  let coins = [
    { name: "$MEMETD", price: 0.05, lastPrice: 0.05, trend: 0.01, volume: 100000, marketCap: 50000 },
  ];
  let positions = [];
  let tradeHistory = [];
  ...
  function updateMarket() {
    coins.forEach(coin => {
      ...
      coin.volume *= (coin.price > coin.lastPrice) ? 1.01 : 0.99; // Adjust volume proportionally to price movement
      coin.marketCap = coin.price * coin.volume; // Update market cap based on the new price and volume
    });
  ...
  function addNewCoin() {
    const newCoin = {
      ...
      volume: 100000, // Set initial volume to ensure market cap starts at 50,000
